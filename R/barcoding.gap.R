...
#b.gap<-barcoding.gap(ref=pineMothCOI,dist="K80")
...
barcoding.gap<-function (ref,dist=dist){ ### raw/k2p,"raw" "k2p", "euclidean"
  ...
  max.y<-max(c(h.inter$counts,h.intra$counts))
    plot(c(0,max.x),c(0,max.y*0.1),type="n",
  ...
  h.inter2<-hist(inter,freq = TRUE,breaks = 100,col = "blue",border = "white",add = TRUE)
  ...
  h.intra2<- hist(intra,freq = TRUE,breaks = 100,col = "red",border = "white",add = TRUE)
  ...
  legend(x = 0.70*max.x,y = 0.10*max.y,
